...
export default function ComingSoon() {
  ...
  return (
    <main className="min-h-screen flex flex-col justify-between bg-gradient-to-b from-[#0F1F20] via-[#152B2C] to-[#1E3A3B] text-white font-sans relative overflow-hidden selection:bg-[#98C4C5] selection:text-[#1E4B4C]">
      ...
      <div className="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] rounded-full bg-[#E8B4B8]/20 blur-3xl pointer-events-none animate-pulse" style={{ animationDuration: "4s" }} />

      {/* Top Brand Header */}
      <header className="w-full max-w-7xl mx-auto px-6 py-6 sm:py-8 flex items-center justify-between z-10">
        <Link href="/" className="flex items-center gap-2 group">
          <span className="text-2xl sm:text-3xl font-bold font-moresugar text-white tracking-wide transition-transform group-hover:scale-105">
            The Abbie Store
          </span>
        </Link>
      ...
      <section className="w-full max-w-4xl mx-auto px-6 py-8 sm:py-12 flex flex-col items-center text-center z-10 space-y-8">
        ...
        <div className="space-y-4">
          <h1 className="text-4xl sm:text-6xl md:text-7xl font-extrabold font-moresugar text-white tracking-tight leading-tight drop-shadow-sm">
          ...
          <p className="max-w-2xl mx-auto text-sm sm:text-lg text-zinc-300 font-sans leading-relaxed">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2.5 max-w-xl pt-2">
          {["📓 Notebooks", "✨ Journals", "📅 Planners", "🎨 Coloring Books", "💼 Business Kit"].map((pill) => (
            <span
              key={pill}
              className="px-3.5 py-1.5 rounded-full bg-white/10 backdrop-blur-xs border border-white/15 text-xs font-bold font-moresugar text-white shadow-2xs"
            >
              {pill}
            </span>
          ))}
        ...
        <div className="w-full max-w-md pt-4">
          {isSubmitted ? (
            <div className="flex items-center justify-center gap-2 p-4 rounded-2xl bg-emerald-950/60 border border-emerald-500 text-emerald-300 font-moresugar text-sm animate-in fade-in zoom-in-95">
              <CheckCircle2 className="w-5 h-5 text-emerald-400 shrink-0" />
              <span>You&apos;re on the VIP list! We&apos;ll notify you when we launch. 🎉</span>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-2.5 p-1.5 rounded-3xl bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl">
              ...
              <button
                type="submit"
                className="px-6 py-3 rounded-2xl bg-[#98C4C5] hover:bg-[#82b6b7] text-[#0F1F20] text-xs sm:text-sm font-bold font-moresugar transition-all duration-200 flex items-center justify-center gap-2 shadow-md hover:shadow-lg active:scale-95 cursor-pointer"
              >
                <Bell className="w-4 h-4 text-[#0F1F20]" />
                <span>Notify Me</span>
              </button>
            </form>
          )}
        ...
        <div className="flex flex-col items-center gap-3 pt-4">
          <span className="text-xs font-bold font-moresugar tracking-wider text-zinc-400 uppercase">
            Follow our journey
          </span>
          <div className="flex items-center gap-4">
            ...
            <a
              ...
              className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/10 hover:bg-gradient-to-r hover:from-[#f09433] hover:via-[#dc2743] hover:to-[#bc1888] hover:text-white text-white border border-white/20 transition-all duration-300 shadow-xs hover:shadow-md text-xs font-bold font-moresugar group backdrop-blur-xs"
            >
      ...
      {/* Footer Info */}
      <footer className="w-full max-w-7xl mx-auto px-6 py-6 border-t border-white/10 text-center sm:text-left flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-sans text-zinc-400 z-10">
